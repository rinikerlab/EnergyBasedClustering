...
class EBC:
    ...
    def _set_proto_radius(self):
        if self._proto_radius is None:
            ...
            if self._verbose:
                print('Search radius set to: ', np.percentile(distances_samples, self._percentile))
   ...
   #     self.w, self.v = w.real, v.real
   #     return v[:, np.argsort(-w)[:self._n_clusters]]
    def _eigen(self):
        w, v = np.linalg.eig(self._diffusion_matrix)
        argsort = np.argsort(-w)
    ...
    def _check_gap(self):
        ...
        if gap < 1e-5:
            ...
            print(f'Your graph might consist of multiple components which can result in unexpected behaviour.')
            print('Consider increasing the number of knn and the temperature.')
        if self._verbose:
    ...
    def plot_graph(self, threshold=5e-3, tau=10, scaling=5, show_labels=False, save=False, savename=None, dpi=120, colormap=None):
        if not self._visualisation_ready:
            self._prepare_coordinates_2D()
        ...
        fig = plt.figure(0, figsize=(20, 20), dpi=dpi, facecolor='white')
        #plt.scatter(coords[:, 0], coords[:, 1], c=population * 100, zorder=2, cmap=plt.cm.plasma, edgecolor='black', s=250)
        plt.scatter(self._states_2D[:, 0], self._states_2D[:, 1], c=-self.energies, s=0.1, cmap=plt.cm.plasma, zorder=-10, alpha=0.6)
        plt.box(False)
        ax = plt.gca()    
